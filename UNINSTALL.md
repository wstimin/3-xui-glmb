...
```bash
mysql -uroot -p -e "DROP DATABASE IF EXISTS \`shiye_management\`; DROP USER IF EXISTS 'shiye'@'127.0.0.1'; DROP USER IF EXISTS 'shiye'@'%'; FLUSH PRIVILEGES;"
```

如果 MySQL root 没有密码：

```bash
mysql -uroot -e "DROP DATABASE IF EXISTS \`shiye_management\`; DROP USER IF EXISTS 'shiye'@'127.0.0.1'; DROP USER IF EXISTS 'shiye'@'%'; FLUSH PRIVILEGES;"
```
...
```bash
mysql -uroot -p -e "DROP DATABASE IF EXISTS \`my_panel\`; DROP USER IF EXISTS 'my_user'@'127.0.0.1'; DROP USER IF EXISTS 'my_user'@'%'; FLUSH PRIVILEGES;"
```

## 8. 是否卸载 Node.js、MySQL、Nginx
