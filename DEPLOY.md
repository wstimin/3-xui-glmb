...
系统会定时检查用户节点状态：本地到期会自动停用；远端 3-xui client 流量用完或被远端停用时，也会同步停用本地节点。因为流量超限被停用的节点续费后，系统会调用 3-xui 官方 `POST /panel/api/clients/resetTraffic/{email}` 接口清空该 client 流量并重新启用，使用户可以尽快恢复连接。建议 3-xui 节点优先填写 API Token，并确认面板版本支持该接口。
