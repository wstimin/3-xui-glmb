...
- 支持节点到期自动停用、远端流量超限自动停用
- 支持流量超限后续费自动调用 3-xui 官方流量重置接口恢复
- 支持自动创建 VLESS 入站
...
本项目按 3-xui 3.4.1 版本开发和测试。其他 3-xui 版本通常也可以接入，但不同版本或魔改版的 API 路径、字段可能存在差异，建议部署后先在后台测试节点连接和同步功能。流量超限续费恢复依赖 3-xui 的 `POST /panel/api/clients/resetTraffic/{email}` 接口，推荐优先使用 API Token 对接。
